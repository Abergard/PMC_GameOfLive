...
void Display();
void DrawMap();
void DrawView(int start_pointX, int start_pointY, float RED, float GREEN,
              float BLUE, GLenum MODE, float scale);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrievInstance,
  ...
  // WS_POPUPWINDOW
  HWND hWnd = CreateWindowEx(WS_EX_CLIENTEDGE, NazwaKlasy, "GameOfLive",
                             WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT,
                             CW_USEDEFAULT, OKNO.x, OKNO.y, NULL, NULL,
                             hInstance, NULL);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case WM_KEYDOWN: {
    ...
    case '2':
      if (editionMode == true) {
        MessageBox(NULL,
                   "Cell with 2 or 3 neighbours will live to next generation\n",
                   NULL, MB_OK);
        USEcolor[ShowAreaNumber(selected_x, selected_y) - 1] = 1;
        USEcolor[ShowAreaNumber(selected_x, selected_y)] = 1;
    ...
    case '3':
      if (editionMode == true) {
        MessageBox(NULL, "Cell with more than 3 neighbours will die", NULL,
                   MB_OK);
        USEcolor[ShowAreaNumber(selected_x, selected_y)] = 1;
        USEcolor[ShowAreaNumber(selected_x, selected_y) + 1] = 1;
        USEcolor[ShowAreaNumber(selected_x, selected_y) - 1] = 1;
    ...
    case '4':
      if (editionMode == true) {
        MessageBox(
            NULL,
            "If empyt cell have excatly 3 live neighbours, becomes a live cell",
            NULL, MB_OK);
        USEcolor[ShowAreaNumber(selected_x, selected_y) + 1] = 1;
        USEcolor[ShowAreaNumber(selected_x, selected_y) + 100] = 1;
        USEcolor[ShowAreaNumber(selected_x, selected_y) - 100] = 1;
  ...
  case WM_SIZE: {
    ...
    glOrtho(-defaultZoom - ZOOM, defaultZoom + ZOOM, -defaultZoom - ZOOM,
            defaultZoom + ZOOM, -1, 1);
